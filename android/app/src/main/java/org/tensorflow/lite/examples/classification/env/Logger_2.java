...
public final class Logger_2 {
  ...
  static {
    ...
    IGNORED_CLASS_NAMES.add(Logger_2.class.getCanonicalName());
  }
  ...
  /**
   * Creates a Logger_2 using the class name as the message prefix.
   *
   * @param clazz the simple name of this class is used as the message prefix.
   */
  public Logger_2(final Class<?> clazz) {
    this(clazz.getSimpleName());
  }

  /**
   * Creates a Logger_2 using the specified message prefix.
   *
   * @param messagePrefix is prepended to the text of every message.
   */
  public Logger_2(final String messagePrefix) {
    this(DEFAULT_TAG, messagePrefix);
  }

  /**
   * Creates a Logger_2 with a custom tag and a custom message prefix. If the message prefix is set to
   *
   * <pre>null</pre>
   *
   ...
   *     being used
   */
  public Logger_2(final String tag, final String messagePrefix) {
    this.tag = tag;
    final String prefix = messagePrefix == null ? getCallerSimpleName() : messagePrefix;
    this.messagePrefix = (prefix.length() > 0) ? prefix + ": " : prefix;
  }

  /** Creates a Logger_2 using the caller's class name as the message prefix. */
  public Logger_2() {
    this(DEFAULT_TAG, null);
  }

  /** Creates a Logger_2 using the caller's class name as the message prefix. */
  public Logger_2(final int minLogLevel) {
    this(DEFAULT_TAG, null);
  ...
  private static String getCallerSimpleName() {
    ...
    return Logger_2.class.getSimpleName();
  }
